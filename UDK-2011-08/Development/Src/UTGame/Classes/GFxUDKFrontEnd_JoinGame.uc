...
final function ForceLANOption( int PlayerIndex )
{
	...
	MenuItemDataStore.GetResourceProviders('GameModeFilter', Providers);
	for ( i = 0; i < Providers.length; i++)
	{
		Provider = UTUIDataProvider_GameModeInfo(Providers[i]);
		if (Provider.GameSearchClass == "UTGameSearchDM")
		{
			if (MenuItemDataStore.GameModeFilter != i)
			{
				MenuItemDataStore.GameModeFilter = i;		
...
function int GetGameTypeSearchProviderIndex( optional string GameClassName )
{
	...
	if ( GameClassName == "" )
	{
		...
		GameClassName = GameSettings.GetPropertyAsStringByName('CustomGameMode');
	}
